...
# Backup
Write-Host "Backing up to $folderPath" -ForegroundColor green
...
exitOnError -Params "Error Backing up to $folderPath" 

# Compress
$zipPath = -join($folderPath,".7z")
Write-Host "Compressing Backup to file" $zipPath -ForegroundColor green
7z a $($zipPath) "$folderPath\*"
exitOnError -Params "Error Compressing Backup to file $zipPath"

# Copy To AWS
Write-Host "Pushing $zipPath to s3" -ForegroundColor green
aws s3 cp $zipPath "s3://$($args[1])"
exitOnError -Params "Error pushing $zipPath to s3"
